...
async function handler(request: NextRequest, user: any) {
  try {
    const requestedApprovals = await ApprovalModel.findAll({ requested_by_id: user.userId });
    const reviewerApprovals = await ApprovalModel.findAll({ reviewer_id: user.userId });
    
    const approvals = [...requestedApprovals, ...reviewerApprovals];
    ...
    return handleError(error, 'Personal approvals');
  }
}
