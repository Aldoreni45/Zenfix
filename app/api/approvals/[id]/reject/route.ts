...
import { sendApprovalRejectedNotification } from '@/lib/api-helpers/notification-helper';
...
async function handler(request: NextRequest, user: any, id: string) {
  try {
    ...
    if (user.role === 'employee') {
      return handleForbidden('Employees cannot reject');
    }

    const body = await request.json();
    const { comments, reason, change_requests } = body;
    ...
    await logActivity({
      ...
      description: `Approval ${updatedApproval.approval_type} rejected`,
      metadata: { reason },
      request,
    });
    ...
    if (updatedApproval.requested_by_id) {
      await sendApprovalRejectedNotification(updatedApproval.requested_by_id, updatedApproval.approval_type, updatedApproval.numeric_id.toString());
    }
