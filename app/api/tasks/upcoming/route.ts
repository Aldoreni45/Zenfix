...
export async function GET(request: NextRequest) {
  return requireAuth(async (req, user) => {
    try {
      ...
      // Day boundaries at UTC midnight so the range is independent of the
      // server timezone. `startDate` is the (inclusive) start day and the
      // window covers `days` full calendar days after it.
      const fallbackStart = new Date(Date.UTC(now.getUTCFullYear(), now.getUTCMonth(), now.getUTCDate()));
      const startDate = (dateParam && parseDueDateUTC(dateParam)) || fallbackStart;
      ...
      return handleError(error, 'Upcoming tasks');
    }
  })(request);
}