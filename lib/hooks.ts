...
export function useTaskHistoryDailyDetail(date: string | null, enabled = true) {
  const endpoint = date ? apiEndpoints.taskHistoryDailyDetail(date) : '';
  return useApi<any>(endpoint, null, [date], enabled && !!date);
}
